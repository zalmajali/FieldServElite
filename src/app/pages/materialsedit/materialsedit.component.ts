...
@Component({
  selector: 'app-materialsedit',
  templateUrl: './materialsedit.component.html',
  styleUrls: ['./materialsedit.component.scss'],
})
export class MaterialseditComponent  implements OnInit {
  @Input() inspId: string | any;
  @Input() inspctionType: string | any;
  @Input() woMaterialId: string | any;
  ...
  public materials_edit: any;
  public selectValues: any;
  public materials_reset: any;
  public materials_name_edit_succ: any;
  public materials_name_edit_failed: any;
  public materials_name_edit_failed_data: any;
  public returnEditMaterialsResultData: any;
//information
  public returnWoItemInfoData: any;
  public operationWoItemInfoResult: any;
  public returnArrayActiveIngredientsWoUtemFromServer: any;
  public returnArrayWoItemTargetPestsFromServer: any;
  //mat name
  ...
  initialiseTranslation(){
    ...
    this.translate.get('edit_materials').subscribe((res: string) => {
      this.pageTitle = res;
    });
    ...
    this.translate.get('materials_edit').subscribe((res: string) => {
      this.materials_edit = res;
    });
    this.translate.get('materials_name_edit_succ').subscribe((res: string) => {
      this.materials_name_edit_succ = res;
    });
    this.translate.get('materials_name_edit_failed').subscribe((res: string) => {
      this.materials_name_edit_failed = res;
    });
    this.translate.get('materials_name_edit_failed_data').subscribe((res: string) => {
      this.materials_name_edit_failed_data = res;
    });
  }
  async ngOnInit() {
    ...
    //getAllInformation
    await this.functionItemInformation(this.eventId,this.inspId,this.inspctionType,this.woMaterialId,this.branchId)
  }
  functionItemInformation(eventId:any,inspId:any,inspctionType:any,itemId:any,branchId:any){
    let sendValues = {'eventId':eventId,'inspId':inspId,'branchId':branchId,'woMaterialId':itemId};
    this.inspectionpointsService.materialsInformation(sendValues).then(async data=>{
      this.returnWoItemInfoData = data;
      this.operationWoItemInfoResult = this.returnWoItemInfoData.Error.ErrorCode;
      if(this.operationWoItemInfoResult==1){
        this.materialsName = this.returnWoItemInfoData.Data.itemId;
        await this.selectMaterialNameSelected(this.materialsName);
        this.activeIngredients = this.returnWoItemInfoData.Data.activeIngredients;
        await this.functionGetMaterialsIdInfoSelected(branchId,inspctionType,this.materialsName,this.activeIngredients);
        this.usageUnit = this.returnWoItemInfoData.Data.usageUnit;
        this.materialsQuantity = this.returnWoItemInfoData.Data.quantity;
        await this.checkQuantityNameSelected(this.materialsQuantity);
        this.appRate = this.returnWoItemInfoData.Data.applicationRate;
        await this.checkAppRateSelected(this.appRate);
        this.note = this.returnWoItemInfoData.Data.notes;
        await this.checkNoteSelected(this.note);
        this.appMethod = this.returnWoItemInfoData.Data.applicationMethodId;
        await this.selectAppMethodSelected(this.appMethod);
        this.equipmentName = this.returnWoItemInfoData.Data.equipmentId;
        await this.selectEquipmentNameSelected(this.equipmentName);
        this.locations = [];
        this.targetPests = [];
        this.targetPestsQuantity = []
        if(this.returnWoItemInfoData.Data.locationId.length != 0){
          this.locations = this.returnWoItemInfoData.Data.locationId;
          await this.selectLocationNameSelected(this.locations);
        }else{
          this.selectValues = 0;
          this.customLocations = this.returnWoItemInfoData.Data.customLocation;
          await this.checkCustomLocationsSelected(this.customLocations);
        }
        if(this.returnWoItemInfoData.Data.targetPestId.length!=0){
          this.returnArrayWoItemTargetPestsFromServer = this.returnWoItemInfoData.Data.targetPestId;
          for(let i = 0; i < this.returnArrayWoItemTargetPestsFromServer.length;i++) {
            this.targetPests[i] = this.returnArrayWoItemTargetPestsFromServer[i].targetPestsId;
            this.targetPestsQuantity[i]=[];
            this.targetPestsQuantity[i][0] = this.returnArrayWoItemTargetPestsFromServer[i].targetPestsId;
            this.targetPestsQuantity[i][1] = this.returnArrayWoItemTargetPestsFromServer[i].targetPestQuantity;
            this.selectTargetPestsSlected(this.targetPests);
          }
        }else{
          this.errorTargetPsts = "ionItemStyleError";
          this.targetPests[0] = 0;
          this.targetPestsQuantity[0] = []
          this.targetPestsQuantity[0][0]=0
          this.targetPestsQuantity[0][1]="";
          this.selectTargetPestsSlected(this.targetPests);
        }
      }
    }).catch(error=>{
      this.functionItemInformation(eventId,inspId,inspctionType,itemId,branchId)
    });
  }
  async selectMaterialNameSelected(val:any){
    this.errorMaterialsName = "ionItemStyleSuccess";
    this.isErrorMaterialsName = 1;
    this.materialsName = val;
  ...
  selectEquipmentNameSelected(val:any){
    this.errorEquipmentName = "ionItemStyleSuccess";
    this.isErrorEquipmentName = 1;
  ...
  selectAppMethodSelected(val:any){
    this.errorAppMethod = "ionItemStyleSuccess";
    this.isErrorAppMethod = 1;
    this.appMethod = val;
    if(this.appMethod == "" || this.appMethod == undefined || this.appMethod == 0){
  ...
  functionGetMaterialsIdInfo(branchId:any,inspctionType:any,itemId:any){
    ...
    this.inspectionpointsService.materialsInformationByItem(sendValues).then(async data=>{
      ...
      if(this.operationMaterialsInfoByIdResult==1){
        ...
        if(this.returnMaterialsInfoByIdData.Data.equipmentId!=0){
          this.errorEquipmentName = "ionItemStyleSuccess";
        ...
        this.locations = [];
        this.locations[0]= this.returnMaterialsInfoByIdData.Data.locationId;
        await this.selectLocationNameSelected(this.locations);
  ...
  functionGetMaterialsIdInfoSelected(branchId:any,inspctionType:any,itemId:any,active:any=[]){
    let sendValues = {'branchId':branchId,'inspctionType':inspctionType,'itemId':itemId};
    this.inspectionpointsService.materialsInformationByItem(sendValues).then(async data=>{
      this.returnMaterialsInfoByIdData = data;
      this.operationMaterialsInfoByIdResult = this.returnMaterialsInfoByIdData.Error.ErrorCode;
      if(this.operationMaterialsInfoByIdResult==1){
        ...
        for(let i = 0; i < this.returnArrayActiveIngredientsFromServer.length;i++) {
          ...
          this.returnActiveIngredientsArray[i]['name'] = this.returnArrayActiveIngredientsFromServer[i].name;
          this.returnActiveIngredientsArray[i]['activePercent'] = this.returnArrayActiveIngredientsFromServer[i].activePercent;
          this.returnActiveIngredientsArray[i]['check'] = false;
          if(active.includes(this.returnActiveIngredientsArray[i]['id']))
            this.returnActiveIngredientsArray[i]['check'] = true;
        }
      }
    }).catch(error=>{
      this.functionGetMaterialsIdInfoSelected(branchId,inspctionType,itemId,active)
    });
  }
  ...
  selectTargetPestsSlected(val:any){
    this.errorTargetPsts = "ionItemStyleSuccess";
    this.isErrorTargetPsts = 1;
    this.targetPests = val;
  ...
  functionEditNewMaterials(){
    ...
    let sendValues = {'zoneId':this.zoneId,'eventId':this.eventId,'inspId':this.inspId,'woMaterialId':this.woMaterialId,'itemId':this.materialsName,'usageUnitName':this.usageUnit,'quantityVal':this.materialsQuantity,'equipmentId':this.equipmentName,'appMethodId':this.appMethod,'locationsId':this.locations,
    ...
    this.inspectionpointsService.editMaterials(sendValues).then(async data=>{
      this.returnEditMaterialsResultData = data;
      let errorData = this.returnEditMaterialsResultData.Error.ErrorCode;
      if(errorData == 1){
        this.displayResult(this.materials_name_edit_succ);
        this.modalController.dismiss({
          "key":1
        })
      }else if(errorData == 2){
        this.displayResult(this.materials_name_edit_failed);
